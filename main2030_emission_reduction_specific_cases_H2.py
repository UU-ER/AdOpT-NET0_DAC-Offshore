            ...
            for reduction in emission_targets:
                ...
                for period in model.periods:
                    ...
                    for node in model.periods[period].node_blocks:
                        netw = "hydrogenPipelineOffshore"
                        b_netw = model.periods[period].network_block[netw]
                        for arc in b_netw.set_arcs:
                            b_netw.arc_block[arc].var_size.fix(0)

                        h2_networks = ["hydrogenPipelineOnshore_new",
                                       "hydrogenPipelineOnshore_re"]

                        for netw in h2_networks:
                            b_netw = model.periods[period].network_block[netw]
                            for arc in b_netw.set_arcs:
                                b_netw.arc_block[arc].dis_installation[0].binary_indicator_var.fix(0)
                                b_netw.arc_block[arc].dis_installation[1].binary_indicator_var.fix(1)
                ...
                for period in model.periods:
                    ...
                    for node in model.periods[period].node_blocks:
                        ...
                        for netw in h2_networks:
                            b_netw = model.periods[period].network_block[netw]
                            for arc in b_netw.set_arcs:
                                b_netw.arc_block[arc].dis_installation[0].binary_indicator_var.unfix()
                                b_netw.arc_block[arc].dis_installation[1].binary_indicator_var.unfix()
                                print(f"fixing {netw}")
